if (typeof targetInput && typeof buttonActive && typeof state) {
  var targetInput = null;
  var buttonActive = null;
  var state = null;
};
const params = new URLSearchParams(window.location.search);
const capAlt = params.get('cap');
const pagAlt = params.get('pag');
...
const objetoHtmlDefault = {
  ...
  cap_1: {
    id: '',
    defaultButtonMod: { pag: ['3', '4', '5', '8', '6', '30'] },
    class: 'fixed-bottom d-flex align-items-center justify-content-center w-100 mb-2',
    chields: {
      div_parent: {
        class: 'popup divParent justify-content-center',
...
function makeKeyboard() {
  ...
  Object.keys(objetoHtmlDefault).map(caps => {
    if (`cap_${typeof numberOfChapterOrPage !== 'undefined' ? numberOfChapterOrPage(0) : capAlt
    }` == caps && objetoHtmlDefault[caps]?.defaultButtonMod?.pag.includes(typeof numberOfChapterOrPage !== 'undefined' ? numberOfChapterOrPage(1) : pagAlt)) {
